...
Define the Excel output contract for Winston-Lutz sessions: paired `.xltx` + `.xlsx` per session, summary sheet with 24 named cells (the app↔MyQA contract), and per-image sheets sourced from `keyed_image_details`.

## Requirements

### Requirement: Per-session paired xltx + xlsx

For every analysis run (Simple or Advanced), the system SHALL create a session output folder at `/out/<CATEGORY>/<MACHINE_DISPLAY>/<PYLINAC_SUBFOLDER>/WL/<MACHINE>_WL_<RUNFOLDER_NAME>/` where `CATEGORY` defaults to `"Clinical QA"`, `MACHINE_DISPLAY` is the machine's `display_name` (e.g. `"LA2 (TrueBeam)"`), and `PYLINAC_SUBFOLDER` defaults to `"Pylinac"`. The folder SHALL contain two files: a copy of `templates/winston_lutz.xltx` renamed to `<MACHINE>_WL_<RUNFOLDER_NAME>.xltx`, and a filled `<MACHINE>_WL_<RUNFOLDER_NAME>.xlsx` produced by loading that copy via openpyxl, setting the named cells, and saving as `.xlsx`.

#### Scenario: First run for a session

- **WHEN** Simple mode runs for machine `LA2` (display_name `"LA2 (TrueBeam)"`) against runfolder `2026-06-16_143022`
- **THEN** the folder `/out/Clinical QA/LA2 (TrueBeam)/Pylinac/WL/LA2_WL_2026-06-16_143022/` is created, containing `LA2_WL_2026-06-16_143022.xltx` (verbatim copy of the template) and `LA2_WL_2026-06-16_143022.xlsx` (filled)
