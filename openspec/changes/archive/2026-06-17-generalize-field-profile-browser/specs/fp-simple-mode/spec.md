...
Replaced by "Cascading folder browser" (the page is decoupled from per-machine dicom_roots; the physicist navigates to a folder via cascading selectboxes instead of selecting a machine → runfolder).

### Requirement: Success card

Replaced by "In-browser result + browser download" (the success card no longer shows a server-side output path; instead it shows metrics + a download button serving in-memory xlsx bytes).
